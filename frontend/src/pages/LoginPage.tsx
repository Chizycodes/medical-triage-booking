		...
		<div className="min-h-screen grid lg:grid-cols-2">
			<div className="relative hidden lg:flex flex-col justify-between bg-brand px-14 py-12 overflow-hidden">
				...
				<div className="relative space-y-6">
					<h2
						className="text-white text-5xl leading-[1.15] tracking-tight"
						style={{ fontFamily: "var(--font-display)" }}
					>
						The right care,
						<br />
						<em className="not-italic text-blue-200">exactly when</em>
						<br />
						you need it.
					</h2>
				...
				<div className="relative flex items-center gap-6">
					{["GDPR Compliant", "NHS Approved", "256-bit Encrypted"].map((badge) => (
						<div key={badge} className="flex items-center gap-1.5">
							...
							<span className="text-blue-200 text-xs font-medium">{badge}</span>
						</div>
					))}
				</div>
			</div>
